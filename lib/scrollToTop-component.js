...
export default class scrollToTopComponent extends React.Component {
	...
	scrollToTop = () => {
		const inkdropState = inkdrop.store.getState();
		if (inkdropState && inkdropState.editor && inkdropState.editor.viewMode === 'preview') {
			const previewEl = window.document.querySelector('.mde-preview');
	...
	scrollToBottom = () => {
		...
		if (inkdropState && inkdropState.editor && inkdropState.editor.viewMode === 'preview') {
			const BOTTOM_OFFSET = 35; // 35 is just an arbitrary number where I feel comfortable
			const previewEl = window.document.querySelector('.mde-preview');
			const wrapperHeight = previewEl.querySelector(':scope > div:first-child').offsetHeight;
			if (previewEl) previewEl.scrollTop = (wrapperHeight - previewEl.offsetHeight) + BOTTOM_OFFSET;
		} else {
			const editor = inkdrop.getActiveEditor().cm;
